...
function locale(url: URL): { language: string; country: string; edition: string } {
  const language = /^[a-z]{2,3}(?:-[A-Z]{2})?$/.test(url.searchParams.get('hl') || '')
    ? url.searchParams.get('hl')!
    : 'en-US';
  const country = /^[A-Z]{2}$/.test(url.searchParams.get('gl') || '')
    ? url.searchParams.get('gl')!
    : 'US';
...
export async function extractGoogleNews(
  ...
  try {
    ...
    if (!(error instanceof ExtractionError) || !['upstream_error', 'source_blocked'].includes(error.code)) throw error;
    ...
    try {
      ...
      htmlBody = await renderPageHtml(htmlEndpoint, dependencies.browser);
