...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    platform = normalizeChoice(
      ...
      ['any', 'youtube'] as const,
      'any',
      'Platform',
    );
    sort = normalizeChoice(
      url.searchParams.get('sort') ?? undefined,
      ['relevance', 'date'] as const,
      'relevance',
      'Sort',
    );
    creator = normalizeVideoCreator(url.searchParams.get('creator') ?? undefined);
  } catch (error) {
  ...
  const resultUrl = new URL('/api/videos', 'https://extractor.sh');
  ...
  if (platform !== 'any') resultUrl.searchParams.set('platform', platform);
  if (sort !== 'relevance') resultUrl.searchParams.set('sort', sort);
  if (creator) resultUrl.searchParams.set('creator', creator);
  resultUrl.searchParams.set('format', format);

  try {
    const search = await runPublicVideoSearch(normalizedQuery, clientKey, env, {
...
export const ALL: APIRoute = () => new Response(
  JSON.stringify({ error: { code: 'method_not_allowed', message: 'Use GET /api/videos.' } }),
  {
    status: 405,
    headers: {
      Allow: 'GET',
