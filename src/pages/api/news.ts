...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  if (limitValue !== null) {
    ...
    if (!/^\d+$/.test(limitValue) || !Number.isInteger(limit) || limit < 1 || limit > 50) {
      return errorResponse(new ExtractionError('invalid_request', 'Limit must be an integer from 1 to 50.', 400));
    }
  }
  ...
  try {
    ...
    timeframe = normalizeChoice(
      url.searchParams.get('timeframe') ?? undefined,
      ['any', '1h', '1d', '7d', '30d'] as const,
      'any',
      'Timeframe',
    );
  } catch (error) {
  ...
  // Keep the public feed anchored to extractor.sh. The fixed upstream News URL
  // is an implementation detail generated by the core, never caller input.
  const resultUrl = new URL('/api/news', 'https://extractor.sh');
  ...
  if (timeframe !== 'any') resultUrl.searchParams.set('timeframe', timeframe);
  resultUrl.searchParams.set('format', format);

  try {
    const news = await runPublicNewsSearch(normalizedQuery, clientKey, env, {
    ...
    if (format === 'markdown') {
      return new Response(news.result.content, {
        headers: successHeaders(news.ttl, 'text/markdown; charset=utf-8'),
      });
    }
    return new Response(JSON.stringify(news.result), {
      headers: successHeaders(news.ttl, 'application/json; charset=utf-8'),
    });
  } catch (error) {
    return errorResponse(error);
  }
...
export const ALL: APIRoute = () => new Response(
  JSON.stringify({ error: { code: 'method_not_allowed', message: 'Use GET /api/news.' } }),
  {
    status: 405,
    headers: {
      Allow: 'GET',
