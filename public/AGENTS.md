...
MCP clients can instead connect to `https://extractor.mcb-software.workers.dev/mcp` and call the read-only `extract_public_url` tool. It accepts the same public URL and output formats, defaults to Markdown, and shares the GET API cache and limits. See `/docs/mcp/` and `/.well-known/mcp/server-card.json`.
