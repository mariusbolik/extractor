...
import { amazonProductAsin, amazonPublicListInfo, amazonSearchQuery, appStoreTrackId, googlePlayPackageId, isBlueskyProfileUrl, isGoogleNewsUrl, isInstagramUrl, isPossibleMastodonStatusUrl, isRedditUrl, isSoundCloudUrl, isSpotifyUrl, isTikTokUrl, isVimeoUrl, isXUrl, isYouTubeUrl, validateTargetUrl, yahooFinanceSymbol } from './url';
...
describe('source URL detection', () => {
  it('recognizes supported platforms without matching lookalike hosts', () => {
    ...
    expect(yahooFinanceSymbol(new URL('https://finance.yahoo.com/quote/AAPL/'))).toBe('AAPL');
    expect(yahooFinanceSymbol(new URL('https://uk.finance.yahoo.com/quote/%5EGSPC/history/'))).toBe('^GSPC');
    expect(yahooFinanceSymbol(new URL('https://finance.yahoo.com/quote/BTC-USD/'))).toBe('BTC-USD');
    expect(yahooFinanceSymbol(new URL('https://finance.yahoo.com/markets/stocks/'))).toBeNull();
    expect(yahooFinanceSymbol(new URL('https://finance.yahoo.com.example.org/quote/AAPL/'))).toBeNull();
  });
  ...
  it('recognizes exact public Amazon lists and storefronts', () => {
    expect(amazonPublicListInfo(new URL('https://www.amazon.de/hz/wishlist/ls/ABC12345?ref_=wl_share'))).toEqual({
      kind: 'wishlist', id: 'ABC12345',
    });
    expect(amazonPublicListInfo(new URL('https://www.amazon.com/shop/example/list/123'))).toEqual({
      kind: 'storefront', id: 'example',
    });
    expect(amazonPublicListInfo(new URL('https://amazon.com.example.org/shop/example'))).toBeNull();
  });
});
