...
describe('Google News adapter', () => {
  ...
  it('moves to public HTML when the feed stalls', async () => {
    const html = `<html><body><c-wiz>
      <div class="vr1PYe">Fast Publisher</div>
      <a class="JtKRv" href="./read/fast-story">Fast HTML story</a>
    </c-wiz></body></html>`;
    let calls = 0;
    const fetcher = vi.fn(async () => {
      calls += 1;
    ...
    expect(fetcher).toHaveBeenCalledTimes(2);
    expect(result).toMatchObject({ method: 'google-news-html', type: 'feed', source: 'google-news' });
    expect(result.items?.[0]?.title).toBe('Fast HTML story');
  });
