...
function mastodonStatusResult(body: string, url: URL, oembed: OembedData): ExtractionResult | null {
  ...
  const hashtags = (Array.isArray(status.tags) ? status.tags : [])
    .map((tag) => text(record(tag)?.name).replace(/^#/, ''))
    .filter(Boolean)
    .slice(0, 50);
  const likeCount = count(status.favourites_count);
  const replyCount = count(status.replies_count);
  const repostCount = count(status.reblogs_count);
