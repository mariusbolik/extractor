...
function structuredVariant(value: unknown, currency: string | null) {
  ...
  const rawCompareAtPrice = minorPrice(variant.compare_at_price, currency);
  const compareAtPrice = rawCompareAtPrice !== null && (price === null || rawCompareAtPrice > price)
    ? rawCompareAtPrice
    : null;
  ...
  return {
    ...
    ...(compareAtPrice !== null ? {
      compareAtPrice,
      compareAtPriceDisplay: priceDisplay(compareAtPrice, currency),
    } : {}),
    ...(typeof variant.available === 'boolean' ? { available: variant.available } : {}),
  };
}

function productTags(value: unknown): string[] {
  const values = Array.isArray(value)
    ? value
    : typeof value === 'string' ? value.split(',') : [];
  return [...new Set(values.map((tag) => typeof tag === 'string' ? tag.trim() : '').filter(Boolean))].slice(0, 50);
}
...
function productItem(product: ShopifyProduct, storeUrl: URL, currency: string | null): ExtractedItem | null {
  ...
  const directCompareAtPrice = minorPrice(product.compare_at_price ?? product.compare_at_price_min, currency);
  const firstPrice = directPrice ?? variants.find((variant) => variant.available !== false && variant.price !== undefined)?.price
    ...
    ?? null;
  const candidateCompareAtPrice = directCompareAtPrice
    ?? variants.find((variant) => variant.available !== false && variant.compareAtPrice !== undefined)?.compareAtPrice
    ?? variants.find((variant) => variant.compareAtPrice !== undefined)?.compareAtPrice
    ?? null;
  const compareAtPrice = candidateCompareAtPrice !== null
    && (firstPrice === null || candidateCompareAtPrice > firstPrice)
    ? candidateCompareAtPrice
    : null;
