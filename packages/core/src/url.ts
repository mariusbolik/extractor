...
export function yahooFinanceSymbol(url: URL): string | null {
  const hostname = url.hostname.toLowerCase();
  if (hostname !== 'finance.yahoo.com' && !/^[a-z]{2}(?:-[a-z]{2})?\.finance\.yahoo\.com$/.test(hostname)) {
    return null;
  }

  // Only intercept quote and history pages whose meaning maps cleanly to the
  // market snapshot returned by this adapter. Other Yahoo Finance pages remain
  // ordinary webpages instead of returning surprising data for a different tab.
  const encodedSymbol = url.pathname.match(/^\/quote\/([^/]+)(?:\/history)?\/?$/i)?.[1];
  if (!encodedSymbol) return null;

  let symbol: string;
  try {
    symbol = decodeURIComponent(encodedSymbol).trim().toUpperCase();
  } catch {
    return null;
  }
...
export interface AmazonPublicListInfo {
  kind: 'wishlist' | 'storefront';
  id: string;
}

export function amazonPublicListInfo(url: URL): AmazonPublicListInfo | null {
  const hostname = url.hostname.toLowerCase().replace(/^www\./, '');
  if (!AMAZON_RETAIL_HOSTS.has(hostname)) return null;
  const wishlist = url.pathname.match(/^\/hz\/wishlist\/ls\/([A-Za-z0-9_-]{5,80})\/?$/i)?.[1];
  if (wishlist) return { kind: 'wishlist', id: wishlist };
  const storefront = url.pathname.match(/^\/shop\/([A-Za-z0-9._-]{1,80})(?:\/.*)?$/i)?.[1];
  return storefront ? { kind: 'storefront', id: storefront } : null;
}
