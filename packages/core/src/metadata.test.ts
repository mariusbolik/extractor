...
describe('extractPageMetadataFromDocument', () => {
  ...
  it('reads canonical language and modified timestamps from the existing document', () => {
    const { document } = parseHTML(`<html lang="de-de"><head>
      <meta property="article:modified_time" content="2026-08-01T12:30:00+02:00">
      <title>Localized page</title>
    </head><body></body></html>`);
    expect(extractPageMetadataFromDocument(
      document as unknown as Document,
      'https://example.com/article',
    )).toMatchObject({ language: 'de-DE', modifiedAt: '2026-08-01T10:30:00.000Z' });
  });
